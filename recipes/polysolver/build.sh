...
echo "Build strelka"
#export LD_FLAGS=$LD_FLAGS:$PREFIX/lib
...
cd $SRC_DIR/include/ && \
  ...
  sed -i.bak 's|$(MAKE)|$(MAKE) CFLAGS="-g -Wall -O2 -fPIC -m64 -L'"$PREFIX"'/lib"|g' $SRC_DIR/include/strelka-upstream-v1.0.11/strelka/redist/Makefile && \
  sed -i.bak 's|LDFLAGS :=|LDFLAGS := -L'"$PREFIX"'/lib|g' $SRC_DIR/include/strelka-upstream-v1.0.11/strelka/src/Makefile && \
  cat $SRC_DIR/include/strelka-upstream-v1.0.11/strelka/redist/Makefile && \
  make

#  cat $SRC_DIR/include/strelka-upstream-v1.0.11/strelka/src/Makefile && \
